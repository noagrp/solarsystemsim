(()=>{
  ...
  function legendHTML(){return '<div class="learn-copy" style="grid-column:1/-1"><strong>AU</strong> — Astronomical Unit; Earth’s average distance from the Sun, about 149.6 million km.<br><br><strong>Terrestrial planet</strong> — dense rocky world with a solid surface. <strong>Gas giant</strong> — giant world dominated by hydrogen and helium. <strong>Ice giant</strong> — giant planet with a larger fraction of water-, ammonia- and methane-rich material in its interior.<br><br><strong>Rotation</strong> — spin on an axis. <strong>Orbital period</strong> — time to complete one orbit. <strong>Axial tilt</strong> — angle of the spin axis relative to the orbital plane. <strong>Retrograde</strong> — motion opposite the usual direction.<br><br><strong>Exosphere</strong> — extremely thin outer atmosphere where particles rarely collide. <strong>Magnetosphere</strong> — region controlled by a body’s magnetic field. <strong>Heliosphere</strong> — giant bubble created by the solar wind.<br><br><strong>Dwarf planet</strong> — round body orbiting the Sun that has not cleared its orbital neighborhood.<br><br><strong>Scale note:</strong> this visualization preserves the original artistic spacing, sizes and relative motion. It is intentionally not a true distance, size or time scale model.</div>'}
  function renderTab(tab){tabs.querySelectorAll('button').forEach(b=>b.classList.toggle('active',b.dataset.tab===tab));originalStats.style.display='none';originalDesc.style.display='none';extra.style.display='grid';if(tab==='legend'){document.getElementById('pName').textContent='Legend';document.getElementById('pType').textContent='Terms & scale guide';extra.innerHTML=legendHTML();return}const d=K[currentKey]||{};if(tab==='facts'){extra.innerHTML=`<div class="learn-copy" style="grid-column:1/-1"><ul class="fact-list">${(d.facts||[]).map(x=>`<li>${x}</li>`).join('')}</ul></div>`;return}extra.innerHTML=cards(d[tab])}
